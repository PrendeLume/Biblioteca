var formulario = document.forms['busqueda']['busquedaTitulo'];
formulario.addEventListener('keyup', busqueda);

function busqueda(){
    var papi = document.getElementById("1");
    while(papi.firstChild){
        papi.removeChild(papi.lastChild);
    }
    for (let i = 0; i < datos.libros.length; i++) {
        if( datos.libros[i].titulo.toLocaleLowerCase().includes(formulario.value.toLowerCase())){
